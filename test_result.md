...
## NAV PASS ONE — persistent Ticker shell + league-context-preserving pathways — needs testing
frontend:
  - task: "Persistent global OUT rail (HOME·RECAP·NEXT·STATS) on all detail routes via shared Screen wrapper; preserve league context on player ACROSS links; un-gate Stats leaders for non-NHL"
    file: "/app/frontend/src/components/ui.tsx (OutRail in Screen), /app/frontend/src/lib/tabnav.ts, /app/frontend/app/player/[id].tsx, /app/frontend/src/screens/StatsScreen.tsx"
    working: "NA"
    status_history:
      - working: "NA"
        agent: "main"
        comment: "Screen wrapper now renders a persistent OutRail (testIDs out-home/out-recap/out-tonight/out-stats; note NEXT tab key='tonight'). Tapping calls goToTab(key) then router.dismissAll() (fallback navigate('/')) so the underlying tab host shows on the chosen tab. Applies to /team /player /game /league /recap (all use Screen). HomeScreen uses TabScreen (no double rail). player page team/next/last5 links now carry ?league; Stats leaders openPlayer now league-aware with name/pos (was NHL-only). NO league-page redesign. Smoke: rail renders on /team/BOS; deep-link tap dismisses to root (onboarding when no Draft Board — expected)."
agent_communication:
  - agent: "main"
...
## NAV FOUNDATION PASS 1 — Hockey World + universal Search + Follow/Unfollow + CHL parent — needs testing
frontend:
  - task: "Hockey World browse (/hockey), universal Search (/search), Follow/Unfollow on team+player, CHL parent link; global entry icons (globe+search) in TopTabBar and BackBar"
    file: "/app/frontend/app/hockey.tsx, /app/frontend/app/search.tsx, /app/frontend/app/team/[id].tsx, /app/frontend/app/player/[id].tsx, /app/frontend/app/league/[code].tsx, /app/frontend/src/components/ui.tsx (FollowPill), /app/frontend/src/components/TopTabBar.tsx, /app/frontend/src/lib/follows.tsx (toggleTeam/togglePlayer)"
    working: "NA"
    status_history:
      - working: "NA"
        agent: "main"
        comment: "Hockey World at /hockey groups NHL / CHL(WHL,OHL,QMJHL) / NCAA from api.leagues(); each league -> /league/[code]. Universal search at /search uses search_all; results navigate by stable id + league_code (team-> /team/{abbr}?league=, player-> /player/{id}?league=&name=&pos=). Global entry: globe(-> /hockey) + search(-> /search) icons in TopTabBar (tabs) and BackBar (detail routes). Follow/Unfollow: FollowPill on team + player pages via follows.toggleTeam/togglePlayer (personalization only, does not change access). CHL parent chip on WHL/OHL/QMJHL league hub -> /hockey?parent=chl. Context persistence + OUT rail preserved. Smoke verified: /hockey sections render, WHL->hub shows CHL chip."
agent_communication:
  - agent: "main"
    message: "FRONTEND ONLY. Onboard first with ONLY an NHL team (search 'Boston' -> Boston Bruins, skip the rest) so NCAA/WHL/OHL/QMJHL are NOT followed. Then verify DISCOVER-WITHOUT-FOLLOW: (1) From Home tap the globe icon (testID nav-hockey) in the top bar -> Hockey World shows NHL/CHL/NCAA. (2) Travel OHL->CHL->WHL->Kamloops: open OHL league (world-league-ohl), tap the CHL parent chip (league-parent-chl) -> Hockey World, tap WHL (world-league-whl) -> WHL hub, tap Kamloops Blazers -> team page loads (NOT followed, still fully accessible). (3) Search travel: tap search icon (nav-search) -> type 'Boston University' -> tap the NCAA result (search-result-*) -> lands on Boston University NCAA team page. (4) NCAA -> Hockey World -> NHL: from that NCAA team, tap globe -> Hockey World -> NHL -> a team. (5) FOLLOW LOOP: on the Kamloops (WHL, non-onboarded) team page tap the FollowPill (follow-toggle) -> label becomes FOLLOWING; on a player page tap follow-toggle too. Confirm the Draft Board did NOT change until pressing Follow (i.e., before tapping, these were not followed). (6) Regression: OUT rail still present on all detail routes; context-aware tabs still inherit league (from a WHL team, out-recap stays WHL); Back works. Base URL = EXPO_PUBLIC_BACKEND_URL. Do NOT test Team Live Desk mic/PLAY/TALK/STOP. Do NOT flag league page = standings (out of scope)."

## EXPLORE PHASE 1.2 — Progressive hockey-world navigation (data-driven drill-down) — needs testing
backend:
  - task: "GET /api/explore/node?path=... progressive drill-down; data-driven taxonomy (Canada complex tree, Sweden different tree, other countries browse-fallback); honest available/coming_soon; reuse /api/explore/world for entrances"
    file: "/app/backend/explore_taxonomy.py (new), /app/backend/server.py (_explore_by_country helper, /api/explore/node endpoint)"
    working: "NA"
    status_history:
      - working: "NA"
        agent: "main"
        comment: "New /api/explore/node returns ONE next layer per node path. Static trees differ per country (Canada: By Level/Region/League; Sweden: Men's Tiers/Women's/By League). Unknown countries fall back to provider-truth browse of that country's Highlightly leagues. Only nhl/whl/ohl/qmjhl/ncaa are available (code set); everything else coming_soon (no fabricated data). Unknown deep path -> 404. Verified via curl: country:Canada, /level, /level/major-junior (WHL/OHL/QMJHL available), /league (browse), Sweden chooser, Finland browse (7 leagues, 0 live), 404 on bogus."
frontend:
  - task: "Simplified Explore main surface = entrances (search + world lens + HOCKEY NATIONS grid + more countries + international chips); generic node screen renders any drill layer; league choice -> League Hub; coming_soon shows SOON + toast; real Back retraces"
    file: "/app/frontend/src/screens/ExploreScreen.tsx (rewrite), /app/frontend/app/explore/node.tsx (new), /app/frontend/src/lib/api.ts (exploreNode + types)"
    working: "NA"
    status_history:
      - working: "NA"
        agent: "main"
        comment: "Explore no longer lists all 176 leagues; it shows country entrances that push /explore/node?path=country:<Name>. Generic ExploreNode screen renders chooser/leagues layers, LIVE/SOON tags, search shortcut, note banner. Smoke-tested full flow: Explore -> Canada -> By Level -> Major Junior (WHL/OHL/QMJHL LIVE). Back + bottom nav intact, no red screens."
agent_communication:
  - agent: "main"
    message: "Test EXPLORE PHASE 1.2. NO auth. Must onboard first (welcome-go -> region-skip -> interests-next -> onboard-search type 'Wild' pick result-team-* -> onboard-skip until main). BACKEND: GET /api/explore/node?path=country:Canada (chooser, 3 node choices), path=country:Canada/level (Pro/Major Junior available nodes; Junior A/University/Youth coming_soon nodes), path=country:Canada/level/major-junior (WHL/OHL/QMJHL all status=available with code), path=country:Canada/level/pro (nhl available, pwhl coming_soon), path=country:Canada/league (browse: >=3 available), path=country:Sweden (chooser, DIFFERENT tree: Men's Tiers + Women's + By League), path=country:Finland (leagues browse, all coming_soon), path=country:Canada/bogus/deep -> 404. GET /api/explore/world still returns totals+featured+countries+international. FRONTEND: On EXPLORE tab confirm hero 'Take me somewhere in hockey', '34 countries . 176 leagues . 5 live', HOCKEY NATIONS grid (Canada 3 LIVE, USA 2 LIVE). Tap explore-country-Canada -> Canada chooser. Tap 'By Level' -> levels; 'Major Junior' -> WHL/OHL/QMJHL LIVE; tap WHL (node-choice-whl) -> League Hub loads standings (existing). Back retraces each layer. Tap explore-country-Sweden -> different chooser. Tap a SOON leaf (e.g. Junior A -> BCHL SOON, or Sweden Men's Tiers -> SHL) shows a coming-soon toast/SOON tag, no crash. Confirm Search shortcut present on node screens (node-search). Do NOT test Reels/video, Team V2 desk mic, Best Ticker Now. Base URL = EXPO_PUBLIC_BACKEND_URL."
