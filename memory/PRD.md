...
## Jun 2026 — TEAM PAGE V2 (NHL / Minnesota reference). VERIFIED (iteration_25).
Approved scope: premium mobile redesign of the NHL team page only (Minnesota is the reference impl for Ticker Mobile V2, NOT Minnesota-specific). Non-NHL league team pages intentionally keep the legacy layout — no propagation until phone approval. No change to Reggie/Marc behavior, follows, routing.
Backend (additive, non-fatal): providers/nhl.py `_core_team_stats()` fetches NHL stats REST team/summary for the current season, returns ordered `core_stats` [PP%, PK%, FACEOFF%, GF/GAME, GA/GAME] each {label,value,rank,of} with league rank 1..N (GA/GAME ranked lower-is-better). Returns None (rail omitted) if unavailable — never fabricated. Also added `venue`+`broadcast` to `_sched_card` (same source object).
Frontend app/team/[id].tsx: `if (isNhl) return <NhlTeamV2/>` gate; legacy JSX preserved for non-NHL. NhlTeamV2 = compact identity row (logo46 + name + record/PTS/rank + FollowPill) → desk → grounded read line → swipeable TEAM STATS rail (StatCard: big value, label, rank-colored 'NHL #x' where <=8 green / >=23 red / else dim; snapToInterval) → event-style Next Game (58px logos, date/time, venue·broadcast) → GameLine last/recent (34px logos, prominent score) → scorers/goalie/division/roster (reuse legacy styles). Rail architected so future advanced metrics just extend the core_stats array (no page redesign). Reusable StatCard/GameLine.
TeamDesk.tsx: slimmed PLAY/TALK controls + panel 216→198 so controls don't dominate; faces stay unobstructed (existing lower-third). Logic unchanged.
Verified (iteration_25): V2 renders with real MIN data + ranks; horizontal swipe reveals GF/GA cards; Next Game 'Grand Casino Arena · CHSN', card→/game, opponent→team; Follow toggles; OUT rail HOME/RECAP/NEXT(testID out-tonight)/STATS; breadcrumbs tappable; PLAY/TALK present. Regression: /team/KAM?league=whl still legacy (no stat rail). Non-blocker: RN-web deprecated shadow* warning.
HELD / next (per user): these 5 stats are NOT the final Ticker stats architecture — recover the deeper pregame/matchup/in-game/postgame/Stats-page concepts from prior Ticker work before propagating V2 to other NHL teams then other leagues. Await phone approval before propagation.

## Jun 2026 — TEAM PAGE V2 desk composition (phone review 1 feedback).
Feedback: hosts still too far/small; too many labels. Fix (no new art): pre-cropped assets/images/broadcast-desk.png -> broadcast-desk-tight.png (crop 295,282,1240,690) = tight chest-up medium two-shot; TeamDesk.tsx now uses the tight asset. Labels reduced to ONE line: removed "REGGIE + MARC" top pill and "LIVE DESK" kicker (+ their unused styles); kept the single topic title + the transient ON AIR status. Play/Talk stay compact; all logic unchanged. Stats rail + compact header untouched. Not propagated. Awaiting phone review 2.

## Jun 2026 — TEAM PAGE V2 desk (phone review 2): removed permanent text overlay.
Per feedback: deleted the persistent title Text ("MINNESOTA WILD · ON THE DESK") from TeamDesk — image now has ZERO permanent app text overlay. Removed unused title const/style and dropped fallbackTitle from destructure (prop kept in type for caller compat). Kept: close-up composition, lower-third gradient (contrast only, not text), compact PLAY/TALK/Stop, transient ON AIR/LISTENING/THINKING status (top-right, off the faces). Note: the "THE TICKER / AI SPORTS NETWORK / NO INTERMISSION" wordmark visible between the hosts is baked into the source artwork (video-wall), not an app overlay. Nothing else changed; not propagated.
...
## Jun 2026 — TEAM PAGE V2 internal team navigation (phone review 6, NHL only).
Added TeamNavRail (in app/team/[id].tsx, NHL branch only): compact horizontal, swipeable rail OVERVIEW · GAMES · STATS · ROSTER · NEWS — team-level nav, separate from global bottom tabs. Sits below the insight (nothing over the Reggie/Marc hero). Implementation: single ScrollView with ref + onScroll; section Y offsets captured via onLayout (setOff) for stats/games(Next Game)/roster/news; goTo() scrolls to offset - RAIL_H; active state derived from scroll position (pill highlight + blue text). Sticky duplicate rail rendered as an absolute overlay (v2.stickyWrap, top:0) once scrollY passes the inline rail (nothing over hero — hero is above it). STATS scrolls to Minnesota's own Team Stats rail (NOT global Ticker Stats). Added honest NEWS section (empty state: "No verified news feed connected yet — coming soon"). Duplicate rail testIDs suffixed `-sticky`. Header/hero/controls/insight/Team Stats rail unchanged; breadcrumb space preserved. Minnesota only; not propagated. Verified via screenshots: rail visible, STATS tap scrolled + rail pinned to top with STATS active. NEXT (after approval): Next Game + Pregame/Matchup.
...
## NAVIGATION FORK — Phase 2: CONTENT-IS-NAVIGATION [DONE, verified iteration_28]
Grammar locked: bottom=destination, top=mode (only when needed — not forced equal counts), rail=league/category, then content itself is navigation.
- NEW src/components/TappableCrest.tsx: a team crest that is its own doorway to the Team Page (league-aware), designed to sit INSIDE a game/matchup card so the inner press wins (RN responder) — crest->Team while the rest of the card->Game. Parent tap never swallows crest taps.
- Independent crest->Team targets wired into all 6 matchup surfaces: NhlSlate (home slate, inline nested Pressable + router), GameRail (new league prop -> TappableCrest; RecapScreen/TonightScreen pass league), GameDepth mini-scoreboard, ReelsScreen (My Hockey) final/upcoming cards, RecapScreen FinalCard, TonightScreen LiteMatchup. Card body still -> Game.
- Game Page team columns now carry testID crest-<ABBR> (routing already worked); scorers/stars/key players/goalies -> Player; team columns -> Team. Player Page team crest (player-team) -> Team. Two-way Player<->Team reliable.
- Fixed web-only pointerEvents prop deprecation (moved into style) in app/index.tsx.
- Verified iteration_28: acceptance journey passes without the bottom menu — My Hockey -> player -> Player -> team crest -> Team -> game -> Game -> opponent crest -> Opponent Team -> roster player -> Player; Player<->Team round-trip x2 (Kaprizov/MIN, McDavid/EDM); Back retraces; regression clean (nav tabs, GAMES modes/STATS grammar, standings/team-stat/leader routing, search montreal->Canadiens). testID crest fix applied post-report.
- Tiny 18px 'COMING UP' logos on Home intentionally left game-only (below touch-target size).

## TWO DISCOVERY WORLDS — architecture captured (NOT built) for Phase 3
Doc: /app/memory/navigation_worlds_architecture.md. Youth/Local vs Junior/Elite/Pro is an ECOSYSTEM split (not age). Users can follow BOTH. entity_index.LEAGUE_WORLD classifies every league (all "elite" today); each search entity carries additive `world`. Future: world-aware onboarding (Where are you? -> which world? -> pick), world-aware search ranking with an "All Hockey" escape, and Explore hierarchy World->Country/Region->League/Association/Competition->Team->Player/Game (data-driven, not hard-coded to North America). Phase 3 to build after phone review.

## NAVIGATION FORK — Phase R: GAMES -> REELS [BUILT, awaiting test]
Added REELS as a MODE under the global GAMES destination (NOT a 6th bottom tab). Grammar preserved: GAMES(bottom) -> NEXT|RECAP|REELS|STATS(top mode) -> league/category rail -> collections/content.
REAL DATA SOURCES (reported): Highlightly API (BASE sports.highlightly.net/hockey, key HIGHLIGHTLY_API_KEY in backend/.env, leagueIds in highlightly.HL_LEAGUES). Clips are verified/authorized: NHL/WHL/OHL/QMJHL = YouTube-hosted game packages (source=youtube, youtube_id present -> playable INLINE via YoutubeInline). NCAA = ESPN-sourced (source=espn, youtube_id null -> not embeddable -> in-Ticker modal shows "OPEN VIDEO" fallback). Category signal: NHL/CHL clips uniformly category "match-highlights" -> single GAME HIGHLIGHTS collection; NCAA carries real categories goal/save/overtime-shootout-goal/fight -> GOALS/SAVES/FIGHTS collections (verified counts seen: NCAA goals 24, saves 9, fights 1, highlights 6). NO fabricated video; a collection only appears when verified clips exist.
Backend (server.py):
- GET /api/reels?league=&limit= -> {league, collections:[{key,label,clips}], total, enabled}. Collections derived from real category (REELS_CAT_MAP) + light title-keyword fallback (fights/hat-trick/save/hit/interview); only non-empty collections returned in fixed order GOALS/FIGHTS/BIG HITS/SAVES/INTERVIEWS/GAME HIGHLIGHTS. Each clip enriched: league_code, away_abbr/home_abbr (entity_index.resolve_team_abbr), game_id (best-effort match vs league scoreboard_now by abbrs +/-1 day), playable(bool youtube_id).
- GET /api/reels/search?q=&league=&scope=league|all -> parses category keyword (goals/fights/hits/saves/interviews) + league token + free-text team/player matched against title+teams; scope=all searches every HL league (the "All Hockey" escape). Verified/available video only; honest empty otherwise.
- entity_index.resolve_team_abbr(name,league) added (nickname/substring, accent-insensitive) so clip team names -> canonical abbr for content-is-navigation.
Frontend:
- src/screens/ReelsHub.tsx: SEARCH HIGHLIGHTS input (debounced 220ms) at top + sticky; league rail (LeagueSwitcher) when not searching; horizontal collection rails; clip cards (thumbnail/play, ext badge when not inline-playable, title, crest chips -> Team, GAME chip -> Game). Playback INSIDE Ticker via YoutubeInline modal; non-embeddable -> OPEN VIDEO (Linking). Search results as vertical rows with league tag + crest/game nav. Scope toggle "Searching <LG> · tap for All Hockey" <-> "Searching ALL HOCKEY". No autoplay loops.
- GamesHub segments now NEXT|RECAP|REELS|STATS; renders ReelsHub for reels.
- api.ts: ReelClip/ReelCollection/ReelsFeed types + api.reels + api.reelsSearch.
Compilation architecture captured (NOT built): collections are already computed from clip metadata; future auto-compilations (All fights tonight, Every WHL goal tonight, Top saves, Kaprizov recent goals, Best hits this weekend) generate only when underlying verified clips exist — same category/keyword + team/date filters, extendable server-side.
Verified via screenshot: GAMES->REELS shows NEXT/RECAP/REELS/STATS + league rail + GAME HIGHLIGHTS real clips (CAR/VGK crest chips); NHL-scope search for a WHL team returns honest empty + All Hockey escape. Backend proven for nhl/whl/ncaa incl NCAA categorized collections + resolved game_id. Guardrails: no My Ticker redesign, no migration to Best Ticker Now, nav architecture untouched. STOP for phone review before Best Ticker Now migration.

## NAVIGATION FORK — Phase R.1: REELS product correction [BUILT, awaiting test]
Locks the product boundary: GAME RECAP = one game's video story; REELS = Ticker-CURATED entertainment within the league you're viewing; BETTING IQ (later) = user-directed/custom video intelligence.
1. NO user-built packages in main Reels. Reels is Ticker-curated (collections derived from verified category/keyword data). Users cannot request arbitrary custom compilations (e.g. "all McDavid goals last 10 games"). The compilation ENGINE/architecture is PRESERVED server-side (reels_search still accepts scope param; category/keyword/team/date filters intact) and reserved for a FUTURE Betting IQ premium feature — NOT built now, no CTA added yet.
2. REELS inherits the ACTIVE league (shared useContextLeague across NEXT/RECAP/REELS/STATS). Removed the in-Reels LeagueSwitcher and the All-Hockey scope toggle/link. ReelsHub now shows an active-league badge + "Ticker-curated reels"; the basic search is ACTIVE-LEAGUE ONLY (api.reelsSearch scope hardcoded "league"), placeholder "Search <LG> highlights"; empty state no longer offers All Hockey. Cross-league/global video discovery deliberately lives elsewhere, not in league-specific Reels.
Unchanged: verified-video infra, YoutubeInline playback + OPEN VIDEO fallback, content-is-navigation (crest->Team, GAME/View Game->Game). Backend endpoints unchanged (scope still supported for future use).
Verified via screenshot: switched context to WHL in STATS -> REELS auto-showed WHL reels (Raiders vs Silvertips, EVT/PA crests) with WHL badge + no picker + "Search WHL highlights". STOP for mobile review.

## NAVIGATION FORK — EXPLORE Phase 1: "wander the hockey world" [BUILT, awaiting test]
Rebuilt src/screens/ExploreScreen.tsx as a VISUAL discovery/wander surface (not a text directory, not a rigid World->Region->League tree). Reuses everything (universal search, league context, content-is-nav, team/league routes) — no infra rebuilt.
- Two entrances: SEARCH (reuse -> /search) + WANDER.
- World lens pills: ELITE / JUNIOR+ (default) · ALL HOCKEY · YOUTH / LOCAL (testIDs world-elite/all/youth).
- WANDER (elite/all): horizontal "PICK A LEAGUE" rail of identity cards (curated LEAGUE_META: abbr, full name, region, flag, tint; only leagues present in /api/leagues shown, order nhl/whl/ohl/qmjhl/ncaa). Tapping a league (explore-league-<code>) sets context league + opens an INLINE drill panel (reanimated FadeInDown) with real team crests (from nhl/leagueStandings, deduped) as a horizontal rail; tap crest (explore-team-<abbr>) -> Team Page (content-is-nav, league-aware); "LEAGUE HUB" (explore-hub-<code>) -> /league/<code>; "Surprise me" (explore-surprise) -> random team. Non-rigid: pick-a-league-then-teams, no forced geography path.
- YOUTH / LOCAL: HONEST placeholder — copy explaining it's a different ecosystem coming later + a disabled SAMPLE PATH chip row (Canada > British Columbia > Kamloops > Association) marked COMING SOON + CTA back to elite. NO fabricated youth data. ALL HOCKEY also shows a youth teaser card (explore-youth-teaser).
- Motion: reanimated FadeIn/FadeInDown/Layout; horizontal rails; crest imagery; flags; tint cards.
- Verified via screenshot: landing + WHL drill (real crests BDN/SC/SAS/REG/PA) + youth honest placeholder. Guardrails: didn't touch Team/Player/Game/Recap/Reels or Best Ticker Now or Betting IQ. STOP for mobile review.

## NAVIGATION FORK — EXPLORE Phase 1.1: international scale + mobile density [BUILT, awaiting test]
Source of truth: real Highlightly league inventory (~176 leagues / 34 countries). Backend highlightly.all_leagues() (paged /leagues, cached 24h) + GET /api/explore/world -> country-grouped {featured, countries, international, totals} with honest status per league: AVAILABLE (mapped to a real Ticker code: nhl/whl/ohl/qmjhl/ncaa via _AVAILABLE_BY_HLID; ncaa injected) vs COMING_SOON (real provider league, identity only, code=null). Country flags via _COUNTRY_FLAG; featured hockey nations first (Canada/USA/Sweden/Finland/Czech/Russia/Switzerland/Germany/Slovakia), Europe/World -> International group. NO fabricated teams/players/games/ids for coming-soon.
Frontend ExploreScreen redesigned for DENSITY: dropped bulky league cards. Now header totals line + compact COUNTRY ROWS (flag + name + "n LIVE" tag + count) each a horizontal swipeable rail of compact league CHIPS. Live chip = solid + blue dot + testID explore-league-<code>; coming-soon chip = dashed border + "SOON" + testID explore-soon-<id> -> shows honest inline note (setSoon). Tapping a live chip unfolds real team crests inline (reuses standings) under that country row -> tap crest explore-team-<abbr> -> Team Page; LEAGUE HUB -> /league/<code>; Surprise me. Preserved: "Take me somewhere in hockey", search doorway, ELITE/ALL/YOUTH lenses, inline team unfold, Surprise Me, content-is-nav, Back, honest Youth placeholder. api.exploreWorld() + Explore* types added.
Verified via screenshot: 34/176/5 header; Canada 3 LIVE (OHL/QMJHL/WHL + LNAH/Memorial Cup/U SPORTS SOON), USA 2 LIVE, Sweden/Finland/Czech/Russia rows with SHL/Liiga/DEL/etc SOON; WHL drill real crests; SOON note honest. Guardrails: no video-in-Explore, no Reels redesign, no Best import, no Betting IQ, no Team/Player/Game/Recap changes, youth untouched. STOP for phone review.

## Explore Phase 1.2 — Progressive Hockey World Navigation (June 2026) ✅ shipped, agent-tested (iteration_33), awaiting user phone review
- Redesigned Explore navigation ARCHITECTURE (not just cards) into progressive drill-down so Ticker can survive thousands of leagues without another redesign.
- Backend: data-driven taxonomy `backend/explore_taxonomy.py` + `GET /api/explore/node?path=...` returns ONE next layer per node. Static trees differ per country (Canada: By Level/Region/League; Sweden: Men's Tiers/Women's/By League — no region). Unknown countries fall back to provider-truth "browse this country's leagues". Only nhl/whl/ohl/qmjhl/ncaa AVAILABLE; everything else honest COMING SOON, no fabricated data. Unknown deep path -> 404. `/api/explore/world` reused for country entrances (refactored shared `_explore_by_country`).
- Frontend: `ExploreScreen.tsx` rewritten to entrances only (search + world lens + HOCKEY NATIONS grid + more countries + international chips). New generic `app/explore/node.tsx` renders ANY drill layer (chooser/leagues, LIVE/SOON tags, search shortcut escape-hatch, note banner). Country tap pushes node route -> real Back retraces; bottom nav persists. League choice -> existing League Hub (reuses League->Team->Player/Game). Coming-soon leaves show toast.
- Scope respected: no Reels/video in Explore, no Best Ticker Now import, no Betting IQ, no Team/Player/Game/Recap redesign.
- PENDING: user mobile review — does Explore now feel like Ticker contains the hockey world and make you want to wander (and does the progressive drill-down feel simple, not database-like)?
